...
// Login page methods
auth.get('/login', (req, res) => res.render('pages/auth/login', {
  title: 'Login',
  message: null
}))
auth.post('/login', async (req, res) => {
  await loginUser(req.body.username, req.body.password).then(async (user) => {
    if (user) {
      ...
      res.render('pages/auth/login', {
        title: 'Login',
        message: "The username and password provided do not match our records."
      })
    }
  })
})
...
auth.get('/register', (req, res) => res.render('pages/auth/register', {
  title: 'Register',
  message: null
}))
auth.post('/register', async (req, res) => {
  if (await registerUser(req.body.username, req.body.password)) {
    res.render('pages/auth/register', {
      title: 'Register',
      message: `User "${req.body.username}" has been created.`
    })
  } else {
    res.render('pages/auth/register', {
      title: 'Register',
